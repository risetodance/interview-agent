...
- **文档智能处理**：文档上传、分块、向量化（pgvector HNSW 索引）。
- **混合检索**：向量检索 + pg_search 全文检索，RRF 融合排序提升召回质量；面试出题时同步检索知识库与 Web 搜索（MCP）作为参考。
...
- **用户管理**：注册审批、启用/禁用。
- **AI 模型配置中心**：多供应商凭证管理（MiniMax / GLM / DeepSeek 等任意 OpenAI 兼容 API），主模型 / 小模型角色槽位指派，**运行时热切换无需重启**，连通性探活，视觉能力标记与优先级。
...
- **通知中心**：站内信、微信订阅消息、邮件通知、面试提醒定时调度。
- **会员积分**：等级与 AI 额度管理、每日签到、积分商城。
